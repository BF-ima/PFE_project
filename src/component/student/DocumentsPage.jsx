          ...
          {[...feedbacks].reverse().map((fb, index, arr) => {
            ...
            return (
              <div
                ...
                {/* Status badge per feedback entry */}
                <span
  ...
  return (
    <>
      <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-3">
        ...
        {/* Dropzone / children — hidden when approved */}
        {!isApproved && children}
        {!isApproved && acceptLabel && (
...
const DeliverablesTab = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`${BASE}/api/deadline`).then(r => r.json()),
    ])
      .then(([delivData, dlData]) => {
        ...
        // Set deadline
        if (dlData.deadline) {
          const dl   = dlData.deadline;
          const date = dl.deadline_date?.slice(0, 10) || dl.deadline_at?.slice(0, 10);
          const time = (dl.deadline_time || dl.deadline_at?.slice(11, 16) || "00:00").slice(0, 5);
          setDeadlineDate(`${date}T${time}:00`);
        }
      })
      .catch(console.error)
      .finally(() => setLoading(false));
  ...
  const handleUpload = async (typeKey, file) => {
    ...
    try {
      ...
      formData.append("title", TITLE_MAP[typeKey]); // ← required by controller
  ...
  const submitted = deliverables.length;
  const total     = 3;
    ...
    <div className="space-y-4">
      <div className="bg-white rounded-2xl border border-gray-200 shadow-lg overflow-hidden">
        ...
        <div className="px-14 pb-14 space-y-6">
          ...
          {/* Countdown */}
          <div
            className="rounded-xl flex items-center justify-between px-5 py-4 mt-4"
            style={{ backgroundColor: "#E8F4FD", border: "1px solid #B8DCF0" }}>
            <div className="flex items-center gap-2">
              <Clock size={18} style={{ color: "#2D8FBF" }} />
              <div>
                <p className="text-sm font-semibold" style={{ color: "#2D8FBF" }}>
                  Submission Deadline
                </p>
                <p className="text-xs" style={{ color: "#2D8FBF" }}>
                  {deadlineDate
                    ? new Date(deadlineDate).toLocaleString("en-GB", {
