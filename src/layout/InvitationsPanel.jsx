...
import { UserPlus, Check, X, ClockIcon, CheckCircle, XCircle } from 'lucide-react';
...
const InvitationsPanel = ({ 
  ...
  const getInviteStatusStyle = (status) => {
    switch(status) {
      ...
      case "declined":
        return { bg: "bg-gray-50", text: "text-gray-700", icon: XCircle, cardBg: "#f3f4f6" };
      default:
        ...
        {["all", "pending", "accepted", "declined"].map((filter) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
              inviteFilter === filter ? "bg-[#1e3a5f] text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
            }`}
          >
            ...
            {filteredInvitations.map((invitation) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      <h3 className="font-semibold text-gray-800">
                        {invitation.senderName || "Team Leader"}
                      </h3>
                  ...
                  <p className="text-xs text-gray-500 mb-1">{invitation.senderEmail || "No email"}</p>
