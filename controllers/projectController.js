...
if (supervisor.role === "enseignant" && external_supervisor) {
  ...
  if (extUsers.length === 0) {
    return res.status(404).json({ message: "Aucun superviseur externe trouvé avec cet email" });
  }
  external_supervisor_id = extUsers[0].id;  // ← make sure line 45 uses `let` not `const`
}
...
const studentEmail = external_supervisor_id ? (assigned_student_email || null) : null;
    // Lines 47-59 — add assigned_student_email to INSERT
const [result] = await db.execute(
  `INSERT INTO project 
   ...
   VALUES (?, ?, ?, 'PENDING', ?, ?, ?, ?, NOW())`,
  [title, description || null, parseInt(max_students), teacher_id, external_supervisor_id, speciality_id || null, studentEmail]
);
...
exports.getMyProjects = async (req, res) => {
  ...
  try {
    ...
    // Replace the enseignant query (lines 100-106)
query = `
...
exports.updateProjectStatus = async (req, res) => {
  ...
  // Team + internal teacher conversation
  ...
  // Team + external supervisor conversation
  ...
  // Notify external supervisor if present
  const [[projectInfo]] = await db.execute(
    "SELECT external_supervisor_id, title FROM project WHERE id = ?", [id]
  );
  if (projectInfo?.external_supervisor_id) {
    await db.execute(
      `INSERT INTO notification (user_id, type, title, message, is_read, created_at)
       VALUES (?, 'INFO', 'Project Validated', ?, 0, NOW())`,
      [projectInfo.external_supervisor_id, `Your project "${projectInfo.title}" has been validated by the administration.`]
    );
  }
